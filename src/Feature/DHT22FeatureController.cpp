...
#define FEATURE_TYPE_HUMIDITY "humidity"
...
void DHT22FeatureController::PopulateDescription(JsonArray& featureDescriptions)
{
  FeatureController::PopulateDescription(featureDescriptions, FEATURE_TYPE_TEMPERATURE);
  FeatureController::PopulateDescription(featureDescriptions, FEATURE_TYPE_HUMIDITY);
}
...
void DHT22FeatureController::Loop()
{
  ...
  if (_lastWasTemp)
  {
    ...
    if (!isnan(h)) {
      ...
      Serial.printf("DHT22 got data: %f\n", h);
      failed = true;
    }
  }
  else
...
void DHT22FeatureController::PublishMeasureEvent(bool isTemp, float value)
{
  ...
  bool success = _context->GetMessageBus().Publish(_topic, measureEvent);
  Serial.printf("DHT22 publish measure event %d\n", success);
}
