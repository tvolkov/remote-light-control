...
SwitchFeatureController::SwitchFeatureController(int port, DeviceContext* context, int pin, bool onIsHigh) : FeatureController(port, FEATURE_TYPE_SWITCH, context)
{
  _pin = pin;
  _onIsHigh = onIsHigh;
  pinMode(_pin, OUTPUT);
...
void SwitchFeatureController::Handle(JsonObject& command)
{
  ...
  Serial.printf("SWITCH SwitchCommand of port %d with %s.\n", _port, on ? "turn on" : "turn off");
  SetState(on);
}
